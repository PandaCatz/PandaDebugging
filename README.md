...
| 2 | V30MZ CPU + interrupt/bus timing | 🔨 in progress — [specs](docs/hardware/) done + verified; `step()` runs the **full documented 8086/80186 instruction set** (ALU, MOV, shifts, `MUL`/`DIV`, strings + `REP`, control flow, `IN`/`OUT`, `INT`/`IRET` + interrupt delivery). Remaining: wire the CPU into a machine with `core-ws`'s interrupt controller, run WSCpuTest, resolve the cycle-unit question, then add timing |
