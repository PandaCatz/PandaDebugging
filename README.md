...
`cargo clippy --all-targets -- -D warnings`, and **31 tests** all pass in debug
...
| 2 | V30MZ CPU + interrupt/bus timing | 🔨 in progress — [specs](docs/hardware/) done + adversarially verified; CPU register/bus/reset foundation done; decoder/executor next |
...
| `cpu-v30mz` | NEC V30MZ core. Register file, flags, 20-bit segmented addressing, `FFFF:0000` reset, trace-first `CpuBus`, instruction fetch. | 🔨 foundation only — no decode/execute/timing yet |
