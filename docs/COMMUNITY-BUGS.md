...
| 5 | **Monochrome palette pool** | mapping palettes straight to 16 greys instead of the two-stage pool → wrong shading | ✅ `core-ws::palette` — two-stage pool→palette→shade indirection; a pool edit reshades every referencing palette; tested |
| 6 | **Color-zero palette behaviour** | forcing palette index 0 transparent in colour mode; missing the writable translucent-palette / `REG_BACK_COLOR` cases | ✅ `core-ws::palette::color_zero_transparent` — mono (0–3/8–11 opaque, 4–7/12–15 transparent) and colour (all transparent except `REG_BACK_COLOR`); tested |
